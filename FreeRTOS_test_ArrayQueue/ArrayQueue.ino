...
// Define a Array
char cmdSendBuffer[2]={0,0};
char cmdRecvBuffer[1]={0};
int passMessage[3]={0,0,0};
...
void TaskAnalogReadPin0(void *pvParameters){
  ...
  /**
     ...
     */
  xQueueSend(arrayQueue,&cmdSendBuffer,portMAX_DELAY);
  // One tick delay (15ms) in between reads for stability
  vTaskDelayUntil( &xLastWakeTime, 57 );
  }
}
...
void TaskAnalogReadPin1(void *pvParameters){
  ...
  /**
     ...
     * https://www.freertos.org/a00117.html
     */
  xQueueSend(arrayQueue,&(cmdSendBuffer[1]),portMAX_DELAY);
  // One tick delay (15ms) in between reads for stability
  vTaskDelay(256);
  }
}
...
void TaskSerial(void *pvParameters){
  ...
  for (;;){
    if(xQueueReceive(arrayQueue,&cmdRecvBuffer,portMAX_DELAY) == pdPASS ){
      Serial.print("Command from:");
      Serial.print(cmdRecvBuffer[0]);
      Serial.print('\n');
      Serial.print("state A:");
      Serial.println(passMessage[0]);
      Serial.print("state B:");
      Serial.println(passMessage[1]);
